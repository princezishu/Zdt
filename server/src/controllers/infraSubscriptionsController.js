...
const infraSubscriptionAdminMiddlewares = [requireAuth, requirePermission('access_analytics')];
...
router.get('/infra-subscriptions', ...infraSubscriptionAdminMiddlewares, async (req, res, next) => {
  try {
...
router.delete(
  '/infra-subscriptions/:id',
  ...infraSubscriptionAdminMiddlewares,
  async (req, res, next) => {
  try {
