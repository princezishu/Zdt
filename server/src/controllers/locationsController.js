...
const locationsSearchLimiter = createRateLimiter({
  windowMs: 10 * 60 * 1000,
  max: 180,
  message: 'Too many location search requests. Please try again shortly.',
});
...
router.get('/geo/states', locationsSearchLimiter, async (req, res, next) => {
  try {
...
router.get('/geo/districts', locationsSearchLimiter, async (req, res, next) => {
  try {
...
router.get('/geo/subdistricts', locationsSearchLimiter, async (req, res, next) => {
  try {
...
router.get('/geo/places', locationsSearchLimiter, async (req, res, next) => {
  try {
...
router.get('/locations/india-suggest', locationsSearchLimiter, async (req, res, next) => {
  try {
...
router.get('/locations/india-suggest/status', async (req, res, next) => {
  try {
    const status = getIndiaVillageDirectoryStatus();
