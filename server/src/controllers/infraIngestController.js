...
import { upsertTenderIntelligenceRecordFromInfraUpdate } from '../services/tenderIntelligence/sync.js';
...
const infraIngestAdminMiddlewares = [requireAuth, requirePermission('access_analytics')];
...
router.get('/infra-ingest', ...infraIngestAdminMiddlewares, async (req, res, next) => {
  try {
...
router.post('/infra-ingest/:id/ignore', ...infraIngestAdminMiddlewares, async (req, res, next) => {
  try {
...
router.post('/infra-ingest/:id/publish', ...infraIngestAdminMiddlewares, async (req, res, next) => {
  const id = toPositiveInteger(req.params.id);
  if (!id) {
    return res.status(400).json({ error: 'Invalid id' });
  }
  ...
  try {
    ...
    const publishedUpdate = {
      ...
      sourceUrl,
      verificationLevel,
      lastUpdated: resolvedLastUpdated || new Date().toISOString().slice(0, 10),
      createdAt: inbox.published_at || new Date().toISOString(),
    };
    ...
    await upsertTenderIntelligenceRecordFromInfraUpdate(client, publishedUpdate);
