...
router.post('/conversations/:id/messages', requireAuth, chatMessageLimiter, async (req, res, next) => {
  ...
  try {
    ...
    void (async () => {
      ...
      if (conversation.conversation_type === 'team_support') {
        const conversationSubject = String(conversation.subject || '').trim();
        const senderEmail = String(req.user.email || '').trim() || 'n/a';
        const messageSentAt = insertedMessageRow.created_at
          ? new Date(insertedMessageRow.created_at).toISOString()
          : new Date().toISOString();
        const emailSubject = conversationSubject
          ? `Team feedback: ${conversationSubject}`
          : `Team feedback conversation #${conversationId}`;
        const emailBody = [
          'A new team feedback message was submitted.',
          '',
          `Conversation ID: ${conversationId}`,
          `Subject: ${conversationSubject || '(none)'}`,
          `Sender Name: ${senderName || req.user.name || 'User'}`,
          `Sender Role: ${senderRole || req.user.role || 'user'}`,
          `Sender User ID: ${req.user.id}`,
          `Sender Email: ${senderEmail}`,
          `Requester User ID: ${conversation.requester_user_id || 'n/a'}`,
          `Requester Name: ${conversation.requester_name || 'n/a'}`,
          `Property Reference: ${conversation.property_reference || 'n/a'}`,
          `Sent At (UTC): ${messageSentAt}`,
          '',
          'Message:',
          payload.body,
        ].join('\n');

        if (TEAM_FEEDBACK_EMAIL) {
          try {
            await sendEmailMessage({
              to: TEAM_FEEDBACK_EMAIL,
              subject: emailSubject,
              text: emailBody,
              replyTo: senderEmail !== 'n/a' ? senderEmail : undefined,
              deliveryLabel: 'Team feedback email',
            });
          } catch (emailError) {
            console.error('Failed to forward team feedback email:', emailError);
          }
        } else {
          console.warn(
            '[CHAT] Team feedback email skipped because FEEDBACK_NOTIFICATION_EMAIL is not configured.'
          );
        }
      }
    })();
