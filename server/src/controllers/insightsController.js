...
const marketPlacePriceQuerySchema = z.object({
...
router.get('/insights/market/place-price', async (req, res, next) => {
  try {
    const query = marketPlacePriceQuerySchema.parse(req.query || {});
    const data = await getMarketPlacePriceEstimate({
      place: query.place,
    });

    if (!data) {
      return res.status(404).json({ error: 'Place not found. Try a different location.' });
    }
