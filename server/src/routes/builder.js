...
router.post('/company/logo', requireAuth, async (req, res, next) => {
  try {
    ...
    const filename = `company-${ctx.companyId}-${Date.now()}.${decoded.ext}`;
    fs.writeFileSync(path.join(uploadDir, filename), decoded.buffer);

    const urlPath = `/uploads/company-logos/${filename}`;
...
router.post('/company/banners', requireAuth, async (req, res, next) => {
  try {
    ...
    const uploadDir = path.join(process.cwd(), 'uploads', 'company-banners');
    fs.mkdirSync(uploadDir, { recursive: true });
    const nonce = crypto.randomBytes(4).toString('hex');
    const filename = `banner-${ctx.companyId}-${Date.now()}-${nonce}.${decoded.ext}`;
    fs.writeFileSync(path.join(uploadDir, filename), decoded.buffer);

    const urlPath = `/uploads/company-banners/${filename}`;
    const host = req.get('host') || '';
    const absoluteUrl = host ? `${req.protocol}://${host}${urlPath}` : urlPath;
