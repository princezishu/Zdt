...
const listingInteractionSchema = z.object({
  action: z.enum(['click', 'save', 'unsave', 'like', 'unlike']),
...
async function verifyAndConsumePhoneVerification({
  ...
  const consumeRows = await pool.query(
    `
...
async function resolveOptionalUserId(req) {
  ...
  try {
    ...
    return Number(userRows.rows[0].id);
  } catch {
    return null;
  }
}
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      const userRows = await pool.query(
        `SELECT id, name, email FROM users WHERE ${userIdFilter.clause} LIMIT 1`,
        userIdFilter.values
      );
      if (userRows.rowCount === 0) {
        return res.status(404).json({ error: 'User not found' });
      ...
      const nextPassword =
        payload.newPassword || `Temp!${crypto.randomBytes(6).toString('hex')}`;
